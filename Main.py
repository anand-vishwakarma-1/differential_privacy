...
def train(args, train_loader, test_loader, val_loader, model):
    ...
    if args.dp_1:
        ...
        temp_x = np.copy(train_loader[0][0].cpu())
        temp_y = np.copy(train_loader[0][1].cpu())
        temp_y = np.expand_dims(temp_y,axis=1)
        tensor_x = torch.Tensor(temp_x) # transform to torch tensor
        tensor_y = torch.Tensor(temp_y)
        my_dataset = TensorDataset(tensor_x,tensor_y)
        train_loader = DataLoader(my_dataset,batch_size=len(tensor_x)//2)
        model, optimizer, train_loader = privacy_engine.make_private(
            module=model,
            optimizer=optimizer,
            data_loader=train_loader,
            noise_multiplier=1.5,  # Adjust based on privacy requirements
            max_grad_norm=1.0,
        )
    if args.dp_2:
        MAX_GRAD_NORM = 1.2
        ...
        tensor_y = torch.Tensor(temp_y)
        my_dataset = TensorDataset(tensor_x,tensor_y)
        train_loader = DataLoader(my_dataset,batch_size=len(tensor_x))
